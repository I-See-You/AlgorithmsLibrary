...
const int M = 44444;
const ULL hs = 3797;
int chainNo, limit, chainId[M], chainHead[M], posInBase[M], baseArray[M];
int Level[M], LCA[M][22], subSize[M], Tree[M << 2], Qr[M << 2];
int n, in[M];
vi G[M];
...
int query_tree(int node, int b, int e, int i, int j)
{
    ...
    int lf = node << 1;
    int rt = lf | 1;
    return query_tree(lf, b, mid, i, j) + query_tree(rt, mid + 1, e, i, j);
...
{
    if(u == v) return in[u];
    int uchain, vchain, ans = 0;
    uchain = chainId[u];
    vchain = chainId[v];
    while(true){
        uchain = chainId[u];
        if(uchain == vchain){
            ans += query_tree(1, 1, limit, posInBase[v], posInBase[u]);
            break;
        }
        ans += query_tree(1, 1, limit, posInBase[chainHead[uchain]], posInBase[u]);
        u = chainHead[uchain];
        u = LCA[u][0];
    }
    return ans;
...
int query(int x, int y)
{
    ...
    int a = query_up(x, lca);
    int b = query_up(y, lca);
    return a + b - in[lca];
}
