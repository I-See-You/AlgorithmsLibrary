...
int res[N], n, qr, color[N], frq[N], vis[N], Ans, lf, rt, st[N], ed[N], Time, org[N], node[N];
int L[N], LCA[N][22];
vi G[N];
...
int main()
{
    ...
    For(i, 1, n + 1) scani(color[i]); Compress(color, n, 1);
    For(i, 1, n) {
        int x, y; scani2(x, y);
        G[x].pb(y);
        G[y].pb(x);
    ...
    built_LCA();
    Rep(i, qr) {
        int x, y; scani2(x, y); if(L[x] > L[y]) swap(x, y);
        int lca = find_LCA(x, y);
        if(lca == x) Q[i] = data(st[x], st[y], 0  , i);
