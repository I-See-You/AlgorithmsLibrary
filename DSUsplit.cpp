...
pii E[N];
int n, m, qr;
int prOr[N], prAx[N], vs[N], vsId, Tree, axTree, res[N];

int findOr(int x) {
    if(x == prOr[x]) return x;
    return prOr[x] = findOr(prOr[x]);
}
...
int orAxMerge(int x) {
    if(prOr[x] == x) return x;
    vs[x] = vsId;
    return prOr[x] = prAx[x] = findOr(prOr[x]);
}

int findAx(int x) {
    if(vs[x] != vsId) {
        vs[x] = vsId;
        if(prOr[x] == x) prAx[x] = x;
        else prAx[x] = findAx(orAxMerge(prOr[x]));
        return prAx[x];
    }
    if(prAx[x] == x) return x;
    return prAx[x] = findAx(prAx[x]);
}
...
void leftMerge(int bx, const data q) {
    vsId++;
    axTree = Tree;
    int l = q.l;
    int r = min(q.r, (bx + 1) * box - 1);
    for(int c = l; c <= r; c++) MergeAx(c);
    res[q.id] = axTree;
}
...
int main()
{
    ...
    while(t--) {
        scanf("%d %d %d", &n, &m, &qr);
        for(int i = 0; i < box; i++) Q[i].clear();
        for(int i = 0; i < m; i++) scanf("%d %d", &E[i].ff, &E[i].ss);
        for(int i = 0; i < qr; i++) {
            int l, r; scanf("%d %d", &l, &r); l--; r--;
            Q[l / box].pb(data(l, r, i));
        }
        for(int i = 0; i < box; i++) {
            if(Q[i].empty()) continue;
            sort(all(Q[i]));
            for(int j = 0; j <= n; j++) prOr[j] = j;
            Tree = n;
            int r = (i + 1) * box;
