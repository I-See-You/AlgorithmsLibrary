...
int n, qr, lf, rt, st[N], ed[N], Time, org[N], node[N], vis[N];
LL Ans, res[N];
int L[N], LCA[N][20];
vi G[N], C[N];

int box[N], bit[N], frq[N];

void dfs(int x, int F, int d = 0) {
    L[x] = L[F] + 1; LCA[x][0] = F;
    st[x] = ++Time; org[Time] = d; node[Time] = x;
    for(int j = 0; j < G[x].size(); j++) if(G[x][j] != F) dfs(G[x][j], x, C[x][j] + 1);
    ed[x] = ++Time; org[Time] = d; node[Time] = x;
}

#define inAnc(x, y) (st[x] <= st[y] && ed[y] <= ed[x])

inline int find_LCA(int x, int y) {
    if(inAnc(x, y)) return x; int log = log2(max(L[x], L[y]));
    for(int i = log; i >= 0; i--) if(LCA[x][i] && !inAnc(LCA[x][i], y)) x = LCA[x][i];
    return LCA[x][0];
}

inline void built_LCA() {
    memset(LCA, 0, sizeof LCA); L[0] = -1; dfs(1, 0);
...
int ans_me() {
    ...
    for(int i = max(1, base * ib); i < base * (ib + 1); i++) {
        if(bit[i] == 0) {
            return i - 1;
        }
    }
    cerr << " The code should not come here\n";
    assert(true);
}
...
int main()
{
    scani2(n, qr); base = sqrt(n + qr) + 1;
    For(i, 1, n) {
        int x, y, w; scani3(x, y, w); if(w > n) w = n + 2;
        G[x].pb(y); C[x].pb(w);
        G[y].pb(x); C[y].pb(w);
    }
    built_LCA(); box[0]++; bit[0]++;
    Rep(i, qr) {
        int x, y; scani2(x, y); if(st[x] > st[y]) swap(x, y);
        int lca = find_LCA(x, y);
        if(lca == x) Q[i] = data(st[x] + 1, st[y], 0      , i);
        else         Q[i] = data(ed[x], st[y], st[lca], i);
    } sort(Q, Q + qr);
    Rep(i, qr) MO(i);
    Rep(i, qr) write(res[i]);
    return 0;
}
