...
public:
    struct edge {
        ...
        long long c;
        edge() {}
        edge(int _u, int _v, int __next, long long _c, int _w) {
            u = _u, v = _v, _next = __next, c = _c, w = _w;
        }
    } edgeList[10004];
    int ee, start[111], nn, idx[111][111];
    int addEdge(int u, int v, long long cuv, long long cvu = 0, int w = 0) {
        edgeList[ee] = edge(u, v, start[u], cuv, w);
        start[u] = ee;
        idx[u][v] = ee++;
        edgeList[ee] = edge(v, u, start[v], cvu, -w);
        start[v] = ee;
        return idx[v][u] = ee++;
    }
    int source, sink, level[111];
    ...
    long long dfs(int src, long long _minCap) {
        ...
        for(int i = start[src]; i != -1; i = edgeList[i]._next) {
            ...
            y = dfs(edgeList[i].v, min(edgeList[i].c, _minCap -x));
            x += y;
            edgeList[i].c -= y;
            edgeList[i ^ 1].c += y;
            if(x == _minCap) break;
    ...
    long long flowEdge(int i, int j) {
        return edgeList[ idx[i][j] ].c;
    }
    void updateEdge(int i, int j, long long tc){
        edgeList[ idx[i][j] ].c += tc;
    }
    void changeEdge(int i, int j, long long tc) {
        edgeList[ idx[i][j] ].c = tc;
    }
...
int main()
{
    ...
    while(t--) {
        int Rsum = 0, Csum = 0;
        scani2(n, m);
        Rep(i, n) Rsum += scani(R[i]);
        Rep(i, m) Csum += scani(C[i]);
        int source = n + 1;
        int sink = source + 1;
        int split = sink + 1;
        FLOW.init(source, sink, split * 2 + 3);
        Rep(i, n) {
            FLOW.addEdge(source, i, R[i]);
        }

        Rep(i, n) {
            Rep(j, m) {
                FLOW.addEdge(i, j + split, 1);
            }
        }

        Rep(i, m) {
            FLOW.addEdge(i + split, sink, C[i]);
        }

        int flow = FLOW.maxFlow();
        printf("Case %d:", qq++);
        if(flow < Rsum || Rsum != Csum) {
            puts(" impossible");
            continue;
        } pc('\n');
        Rep(i, n) {
            Rep(j, m) {
                if(FLOW.flowEdge(j + split, i) == 0) {
                    res[i][j] = 0;
                }
                else {
                    FLOW.updateEdge(source, i, 1);
                    FLOW.updateEdge(j + split, sink, 1);
                    int cur = FLOW.maxFlow();
                    if(cur) {
                        flow++;
                        res[i][j] = 0;
                    }
                ...
                FLOW.changeEdge(i, j + split, 0);
                FLOW.changeEdge(j + split, i, 0);
            }
        }
        Rep(i, n) {
            Rep(j, m) {
