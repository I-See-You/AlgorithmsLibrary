...
int n, in[N * 20], Root[N * 20], id, qr, res[N * 20], last;
...
int query(int pr, int cr, int b, int e, int i, int j) {
    if(b > j || e < i) return 0;
    if(b >= i && e <= j) return T[cr].c - T[pr].c;
    int mid = b + e >> 1;
    int L = query(T[pr].l, T[cr].l, b, mid, i, j);
    int R = query(T[pr].r, T[cr].r, mid + 1, e, i, j);
    return L + R;
}
...
int main()
{
    ...
    vi cmp; cmp.push_back(-inf); cmp.push_back(inf);
    for(int i = 1; i <= n; i++) cmp.push_back(in[i]);
    Unique(cmp);
    for(int i = 1; i <= n; i++) in[i] = lower_bound(all(cmp), in[i]) - cmp.begin();
    Root[0] = ++id;
    for(int i = 1; i <= n; i++) Root[i] = update(Root[i - 1], 0, cmp.size(), in[i]);
//    for(int i = 1; i <= n; i++) cout << in[i] << ' '; cout << '\n';
    ...
    for(int i = 0; i < qr; i++) {
        ...
        k ^= last;
        k = upper_bound(all(cmp), k) - cmp.begin();
        res[i] = query(Root[l - 1], Root[r], 0, cmp.size(), k, cmp.size());
        last = res[i];
    }
