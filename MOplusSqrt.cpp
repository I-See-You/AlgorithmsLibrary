...
deque <int> D[N];
int n, qr, in[N], rt, lf, res[N], box[N], org[N];




void Erase(int sz) {
    if(--org[sz] == 0) box[sz / base]--;
}
...
int FindMax() {
    int ib = -1;
    rFor(i, base + 1, 0) if(box[i]) {
        ib = i;
        break;
    }
    if(ib == -1) return 0;
    rFor(i, (ib + 1) * base - 1, ib * base) if(org[i]) return i;
...
int main()
{
    ...
    while(t--) {
        ...
        For(i, 1, 1 + n) in[i] += in[i - 1];
        Rep(i, n + 1) in[i] += n;
        base = sqrt(n * 2 + qr) + eps;
        Rep(i, qr) {
            int x, y; scani2(x, y);
            Q[i] = data(x - 1, y, i);
        }
